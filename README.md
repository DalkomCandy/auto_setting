...
**매크로 파일 위치·평가기준일·변수일·평가자·리뷰어·매크로 이름 4개**는 그 칸을
벗어나는(다른 칸을 클릭하는) 즉시 저장됩니다 — 창을 닫을 때까지 기다리지 않습니다.
**[지금 저장]** 버튼을 누르면 실제로 저장됐는지 로그에서 바로 확인할 수 있고,
저장이 실패하면(디스크 문제 등) 조용히 사라지지 않고 로그에 경고로 남습니다.
...
`SELECT RANGE` 는 셀 범위를 고르는 **RefEdit** 칸이라 봇이 절대 건드리지 않고,
그 밖의 칸 중 **비어 있는 첫 칸**을 고릅니다.

그래도 틀리면 **고급 › 입력칸 선택**을 `0`, `1`, `2`… 로 직접 지정하세요.

## 「이 이름에 대한 구문이 잘못되었습니다」오류가 뜨고, 확인을 누르면 매크로는 실행됨

`SELECT RANGE`(RefEdit) 칸에 펀드아이디 같은 문자열이 잘못 들어가면 나는 오류입니다.
유효한 셀 범위가 아니라서 Excel 이 이렇게 알려줍니다. 최신 버전에서는 RefEdit 칸을
클래스 이름으로 구분해서 절대 그 칸에 값을 넣지 않도록 고쳤습니다 —
`git pull` 로 받은 뒤에도 계속 나면 **[창 구조 확인]**으로 실제 컨트롤 구성을 보여주세요.
...
- **4단계 시트 비우기** → `Cells.Delete()`, 즉 **Ctrl+A → 우클릭 → 삭제**와 같습니다.
  ...
  이어서 **F_1 이 있던 시트 전체를 흰색으로 채웁니다** — '채우기 없음'만으로는
  테마에 따라 회색조로 보이거나 격자선이 남을 수 있어서, 확실한 흰 배경으로
  덮어 다음 단계부터 채워질 보고서가 깨끗한 캔버스 위에서 시작하게 합니다.
- **종목아이디 0개** → 정상으로 보고 **10·11단계만 건너뜁니다.**
  나머지 단계는 그대로 진행하고 저장하며, 결과에 `9단계 완료 · 2단계 건너뜀 · 저장됨` 으로 남습니다.
- **파일이 이미 열려 있을 때** → 저장할 필요가 없는 실행(**읽기만**, **N단계까지**)은
  파일을 **읽기 전용**으로 엽니다. 그래서 사용자가 확인차 같은 파일을 이미 열어
  둔 상태에서도 "사용 중인 파일" 충돌 없이 그대로 진행됩니다. 반대로 **전체 실행**
  (저장까지 하는 실행)은 쓰기 권한이 필요하므로, 다른 프로그램이 파일을 쓰기
  모드로 잡고 있으면 열기 전에 미리 감지해 명확한 오류로 알려주고 멈춥니다
  (Excel 의 "사용 중인 파일" 대화상자가 뜬 채로 조용히 멈춰버리는 상황을 막기 위해서).
